import { BookOpen, CreditCard, LayoutDashboard, LogOut, Upload, UserCircle, Wallet } from 'lucide-react';
    ...
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          ...
          {isAuthenticated ? (
            <>
              <Link to={isAdmin ? '/admin/dashboard' : '/profile'} className="hidden rounded-2xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 md:flex md:items-center md:gap-2">
                <UserCircle size={16} />
                {user?.name?.split(' ')[0]}
              </Link>
              {isAdmin ? (
                <Link to="/admin/dashboard" className="hidden rounded-2xl bg-brand-600 px-3 py-2 text-sm font-medium text-white hover:bg-brand-700 md:flex md:items-center md:gap-2">
