    ...
    >
      ...
      <div className="px-1 pb-1 pt-4">
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          <p className={`text-lg font-semibold ${Number(resource.price || 0) === 0 ? 'text-emerald-600' : 'text-slate-900'}`}>
            {Number(resource.price || 0) === 0 ? 'Free' : `$${Number(resource.price || 0).toFixed(0)}`}
          </p>
          <button
            type="button"
            onClick={handleAdd}
            disabled={alreadyInCart}
            className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition ${alreadyInCart ? 'bg-slate-100 text-slate-400' : 'bg-brand-500 text-white hover:bg-brand-600'}`}
          >
